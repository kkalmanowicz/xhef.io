...
const useOrderListRealtime = (supabase, userId, loadCurrentOrder, loadSuggestedItems, loadOrderHistory) => {
  ...
  useEffect(() => {
    ...
    const handleOrderItemsChange = (payload) => {
      console.log(`Realtime: Order items change detected (v6${uniqueChannelSuffix})`, payload);
    ...
    const orderItemsChannel = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'order_items', filter: `user_id=eq.${userId}` },
        handleOrderItemsChange
      )
      .subscribe((status, err) => {
        if (status === 'SUBSCRIBED') {
          console.log(`Realtime: Subscribed to order_items changes (hook v6${uniqueChannelSuffix})`);
        } else if (status === 'CHANNEL_ERROR' || status === 'TIMED_OUT') {
          console.error(`Realtime: Order items channel error (hook v6${uniqueChannelSuffix}):`, err);
        } else {
          console.log(`Realtime: Order items channel status (hook v6${uniqueChannelSuffix}):`, status);
        }
      });

    const handleInventoryChange = (payload) => {
      console.log(`Realtime: Inventory items change detected (v6${uniqueChannelSuffix})`, payload);
      loadSuggestedItemsRef.current(false);
    ...
    const inventoryChannel = supabase
      ...
      .subscribe((status, err) => {
         if (status === 'SUBSCRIBED') {
          console.log(`Realtime: Subscribed to inventory_items changes (hook v6${uniqueChannelSuffix})`);
        } else if (status === 'CHANNEL_ERROR' || status === 'TIMED_OUT') {
          console.error(`Realtime: Inventory items channel error (hook v6${uniqueChannelSuffix}):`, err);
        } else {
          console.log(`Realtime: Inventory items channel status (hook v6${uniqueChannelSuffix}):`, status);
        }
      });
      
    const handleOrdersChange = (payload) => {
      console.log(`Realtime: Orders table change detected (v6${uniqueChannelSuffix})`, payload);
    ...
    const ordersChannel = supabase
      ...
      .subscribe((status, err) => {
        if (status === 'SUBSCRIBED') {
          console.log(`Realtime: Subscribed to orders changes (hook v6${uniqueChannelSuffix})`);
        } else if (status === 'CHANNEL_ERROR' || status === 'TIMED_OUT') {
          console.error(`Realtime: Orders channel error (hook v6${uniqueChannelSuffix}):`, err);
        } else {
          console.log(`Realtime: Orders channel status (hook v6${uniqueChannelSuffix}):`, status);
        }
      });

    return () => {
      console.log(`Realtime: Unsubscribing from all order list channels (hook v6${uniqueChannelSuffix})`);
