...
const usePrepListData = (supabase, userId, toast, isMountedRef) => {
  ...
  const handleStockChange = (itemId, change) => {
    setLocalStock(prev => ({
      ...
      [itemId]: Math.max(
        0,
        (prev[itemId] === undefined ? 0 : prev[itemId]) + change
      ),
    }));
  };
      ...
      if (stockDifference > 0 && item.prep_item_ingredients?.length > 0) {
        for (const ingredient of item.prep_item_ingredients) {
          ...
          if (availableStock < requiredAmount) {
            throw new Error(
              `Not enough ${ingredient.inventory_items?.name || 'ingredient'} in inventory. Required: ${requiredAmount}, Available: ${availableStock}`
            );
          }
        }
        ...
        for (const ingredient of item.prep_item_ingredients) {
          ...
          const { error: updateError } = await supabase
            ...
            .update({
              current_stock:
                (ingredient.inventory_items.current_stock || 0) - deductAmount,
...
function PrepList() {
  ...
  const filteredPrepItems = prepItems.filter(
  ...
  useEffect(() => {
    ...
    const prepItemsChannel = supabase
        ...
        payload => {
          ...
          if (isMounted.current) {
            ...
            if (
              ...
              if (
                (payload.old?.status === 'not-needed' &&
                  payload.new?.status !== 'not-needed') ||
                (payload.old?.status !== 'not-needed' &&
                  payload.new?.status === 'not-needed')
              ) {
      ...
      .subscribe((status, err) => {
        if (status === 'SUBSCRIBED')
          console.log(`Subscribed to prep_items${prepChannelSuffix}`);
        if (
          ...
          console.error(
            `Prep Items channel error/closed (${prepChannelSuffix}):`,
            status,
            err
          );
        }
    ...
    const inventoryItemsChannel = supabase
      ...
      .on(
        ...
        payload => {
          console.log(
            'Inventory items change received (for prep list):',
            payload
          );
          ...
          const affectsPrepItem = prepItems.some(pItem =>
            pItem.prep_item_ingredients.some(
              ing => ing.inventory_items.id === changedInventoryItemId
            )
          );
      ...
      .subscribe((status, err) => {
        if (status === 'SUBSCRIBED')
          console.log(`Subscribed to inventory_items${inventoryChannelSuffix}`);
        if (
          status === 'CHANNEL_ERROR' ||
          status === 'TIMED_OUT' ||
          status === 'CLOSED'
          ...
          console.error(
            `Inventory Items (for prep) channel error/closed (${inventoryChannelSuffix}):`,
            status,
            err
          );
        }
    ...
    return () => {
      if (supabase) {
        supabase
          .removeChannel(prepItemsChannel)
          .catch(err =>
            console.error('Error removing prep items channel:', err)
          );
        supabase
          .removeChannel(inventoryItemsChannel)
          .catch(err =>
            console.error('Error removing inventory items channel:', err)
          );
      }
    };
    ...
    <div className="space-y-8 p-4 md:p-6">
      ...
      {isLoading ? (
        ...
        <div className="text-center py-12 text-gray-500 dark:text-gray-400">
          ...
          <p className="mt-2">
            No items need preparation for{' '}
            {selectedStation === 'all'
              ? 'any station'
              : `the ${selectedStation} station`}{' '}
            at this time.
          </p>
        </div>
      ) : (
